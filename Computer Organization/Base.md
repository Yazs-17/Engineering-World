...
以Linux文件系统为例，Linux文件系统会为每个文件分配两个数据结构：索引节点和目录项
- 索引节点（inode），用以记录文件元信息，唯一文件标识，同样占用磁盘空间
- 目录项（dentry），记录文件名，索引节点指针以及其他目录项的层级关联关系，缓存于内存

目录项不等于目录，前者缓存在内存，后者在磁盘

扇区，逻辑块，超级块，索引节点区，数据块区
### 虚拟文件系统

文件系统的种类众多，而操作系统希望对用户提供一个统一的接口，于是在用户层与文件系统层引入了中间层，这个中间层便被称为VFS-虚拟文件系统
